...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    <head>
        ...
        <title>{{ $listDetail->displayName }} - {{ config('app.name', 'Laravel') }}</title>
        ...
        <div class="flex items-center justify-center w-full transition-opacity opacity-100 duration-750 lg:grow starting:opacity-0">
            <main class="flex max-w-[335px] w-full flex-col-reverse lg:max-w-4xl lg:flex-row">
                <div class="text-[13px] leading-[20px] flex-1 p-6 pb-6 lg:p-20 lg:pb-10 bg-[#f9f9f8] dark:bg-[#161615] dark:text-[#EDEDEC] shadow-[inset_0px_0px_0px_1px_rgba(26,26,0,0.16)] dark:shadow-[inset_0px_0px_0px_1px_#fffaed2d] rounded-lg">
                    <h1 class="mb-1 font-medium text-2xl">{{ $listDetail->displayName }}</h1>
                    <p class="mb-4 text-[#706f6c] dark:text-[#A1A09A]">
                        Bestsellers Date: {{ $listDetail->bestsellersDate }} | Updated: {{ $listDetail->updated }}
                    </p>

                    <div class="space-y-8 mt-8">
                        @foreach($listDetail->books as $book)
                    ...
                    @if($listDetail->books->isEmpty())
                        <p class="text-center py-10 text-[#706f6c]">Brak książek na tej liście.</p>
                    @endif
